...
func RefreshSessionCache() {
	...
	defer cancel()
	cmd := TmuxCommandContext(ctx, "list-windows", "-a", "-F", "#{session_name}\t#{window_activity}\t#{window_index}\t#{window_name}")
...
func (s *Session) SetEnvironment(key, value string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	cmd := TmuxCommandContext(ctx, "set-environment", "-t", s.Name, key, value)
	err := cmd.Run()
	if err == nil {
...
func (s *Session) GetEnvironment(key string) (string, error) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	cmd := TmuxCommandContext(ctx, "show-environment", "-t", s.Name, key)
	output, err := cmd.Output()
	if err != nil {
...
func (s *Session) Start(command string) error {
	...
	_ = TmuxCommand("bind-key", "-n", "-T", "root", "C-q",
